...
export const CarInsuranceStage = (props) => {
  const { onChange, carInsurance, savedSplitAmount } = props;
  const [split, setSplit] = useState(null);

  const onSubmitStage = useCallback(() => {
    const carPaymentValue = parseFloat(carInsurance);
    props.stageProps.onSubmitStage(
      numbro(carPaymentValue / split).format({ mantissa: 2 }),
      split,
    );
  }, [split, carInsurance, props.stageProps.onSubmitStage, savedSplitAmount]);
